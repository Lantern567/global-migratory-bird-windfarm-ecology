...
for ax_i, (_, farm) in enumerate(top3.iterrows()):
    ...
    # Set global for Jensen wake efficiency function
    oac.power_curve_global = power_curve

    # Compute efficiency for 72 wind directions
    wd_fine = np.arange(0, 360, 5)
    eta_vals = np.array([
        oac.compute_farm_wake_efficiency(turbs_x, turbs_y, rotor_diams, wd, thrust_curve, v_rated)
        for wd in wd_fine
    ])
